...
export async function renderPipeline(view) {
  ...
  const inputs = card(
    el('div', { class: 'flex items-center gap-2 mb-4' },
      el('h2', { class: 'font-bold' }, '① Inputs we collect'),
  ...
  const prompt = card(
    el('div', { class: 'flex items-center gap-2 mb-3' },
      el('h2', { class: 'font-bold' }, '② Prompt sent to the model'),
      d.model ? pill(d.model) : null,
      d.max_output_tokens ? pill(`${d.max_output_tokens} max tokens`) : null,
      el('button', { class: 'ml-auto text-xs text-accent hover:underline',
        onclick: () => navigator.clipboard.writeText(d.sample_prompt) }, 'copy'),
    ),
  ...
  // ── outputs ──
  const outputs = card(
    el('h2', { class: 'font-bold mb-4' }, '③ Outputs we get back'),
